...
const SchemaProperties = ({
  ...
  const schema = useAppSelector(
    (state) =>
      state.schemas.schemas.filter((s) => s.id === params.id)[0] ||
      ({ name: "" } as any)
  );
  const activeTableArray =
    schema.activeTable && schema.tables
      ? schema.tables.filter((t) => t.id === schema.activeTable)
      : [];
  const activeTable = activeTableArray.length > 0 ? activeTableArray[0] : null;
  const activeTableName =
    activeTableArray.length > 0 ? activeTableArray[0].name : "";
    ...
    if (activeTableName) {
      setActiveSuggestions(
        suggestions.filter((s) =>
          s.body.toLowerCase().includes(activeTableName.toLowerCase())
        )
      );
    } else {
                  ...
                  data={{
                    ...
                    columns: activeTable.columns.map((c) => {
                      return {
                        ...
                        foreignKey:
                          activeTable.foreignKeys.filter(
                            (fk: any) => fk.column === c.name
                          ).length > 0,
                        nullable: c.nullable,
                        index: c.index,
                        autoInc: c.autoInc,
            ...
            <Box>
              ...
              <Box display={"flex"} alignItems={"center"}>
                <Checkbox
                  ...
                  onChange={(e) => toggleRelations(!showRelations)}
                />
                <Typography
                  variant="body2"
                  fontWeight={500}
