...
const SignUp = ({ flowSwitch, googleAuthHandler }: PageProps) => {
  ...
  return (
    <>
      <Box>
        <Typography variant="h2" fontWeight={600}>Register</Typography>
        <Typography variant="subtitle1" my={1}>Hi! Please complete the form to setup your account.</Typography>
      </Box>
      ...
      <Box component="form" mt={4}>
        <FormControl variant="standard" fullWidth>
          ...
          <BootstrapInput placeholder="Enter your full name" id="name" onChange={(e) => {
        ...
        <FormControl variant="standard" sx={{ my: 2 }} fullWidth>
          ...
          <BootstrapInput placeholder="Enter your email" id="email" onChange={(e) => {
      ...
      <Box component="div" display="flex" flexDirection="column" gap={1.8} mt={4}>
        <Button type="primary" label={isLoading ? "" : "Sign up"} onClick={createAccountHandler} icon={isLoading ? <PulseLoader size={10} color="#fff" /> : null} />
      ...
      <Box display="flex" justifyContent="center" alignItems="center" gap={.5} mt={4}>
        ...
        <Link component="button" underline="none" sx={{ fontSize: 14 }} onClick={flowSwitch.bind(null, "login")}>Log in</Link>
      </Box>
    </>
  );
}
