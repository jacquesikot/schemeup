...
import { Box, Button as MuiButton, FormControl, FormControlLabel, InputLabel, Typography, Link, InputAdornment, IconButton } from "@mui/material";
...
import { PageProps } from "../../pages/authenticate-user";
...
const SignIn = ({ flowSwitch, googleAuthHandler }: PageProps) => {
  ...
  return (
    <>
      <Box>
        <Typography variant="h2" fontWeight={600}>Log in</Typography>
        <Typography variant="subtitle1" my={1}>Welcome back! Please enter your details.</Typography>
      </Box>
      ...
      <Box display={"flex"} justifyContent={"space-between"} alignItems={"center"} my={2}>
        <FormControl sx={{ px: 1 }}>
          <FormControlLabel
            checked={staySigned}
            onChange={() => { setStaySigned(value => !value) }}
      ...
      <Box component="div" display="flex" flexDirection="column" gap={1.8}>
        <Button type="primary" label={isLoading ? "" : "Sign in"} onClick={signInHandler} icon={isLoading ? <PulseLoader size={10} color="#fff" /> : null} />
      ...
      <Box display="flex" justifyContent="center" alignItems="center" gap={.5} mt={4}>
        ...
        <Link component="button" underline="none" sx={{ fontSize: 14 }} onClick={flowSwitch.bind(null, "signup")}> Sign up</Link>
      </Box>
    </>
  );
}
